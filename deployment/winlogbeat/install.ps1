##########
# Winlogbeat automatic installation script
...
$version = "5.4.0"
$unzip_file_name = "winlogbeat-$($version)-windows-x86_64"
$zip_file_name = "$($unzip_file_name).zip"
$download_url = "https://artifacts.elastic.co/downloads/beats/winlogbeat/winlogbeat-$($version)-windows-x86_64.zip"
$download_out = "$PSScriptRoot\$($zip_file_name)"
$unzip_out = "C:\Program Files"
#$unzip_out = "$PSScriptRoot\unzip"
$renamed_file_name = "Winlogbeat"
...
function Main {
    ...
    RunPS1 "$($unzip_out)\$($renamed_file_name)\install-service-winlogbeat.ps1"
    WaitForKey
}
